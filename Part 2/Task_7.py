# -*- coding: utf-8 -*-
"""
Created on Thu Jul  7 09:11:45 2022
...
class Pow:
    ...
    def __repr__(self):
        if isinstance(self.p1, Pow)!=True and isinstance(self.p1, Int)!=True and isinstance(self.p1, X)!=True and isinstance(self.p1, Y)!=True:            
            if isinstance(self.p2, Pow)!=True and isinstance(self.p2, Int)!=True and isinstance(self.p2, X)!=True  and isinstance(self.p2, Y)!=True:
    ...
    def simplify(self):
        ...
        if isinstance(simp2, X):
           if isinstance(simp1, Mul):
               ...
               if isinstance(e2, X):
                   return Mul(e1, Pow(X(),Int(2))).simplify()
        if isinstance(simp1, Y):
           if isinstance(simp2, Y): 
               return Pow(Y(),Int(2))
           if isinstance(simp2, Mul):
               e1=simp2.p1
               e2=simp2.p2
               if isinstance(e1, Y):
                   return Mul(e2, Pow(Y(),Int(2))).simplify()
               if isinstance(e2, Y):
                   return Mul(e1, Pow(Y(),Int(2))).simplify()
        if isinstance(simp2, Y):
           if isinstance(simp1, Mul):
               e1=simp1.p1
               e2=simp1.p2
...
class Add:
    ...
    def simplify(self):
        ...
        if isinstance(simp1, Add):
            ...
            if isinstance(simp2, Y):
                if isinstance(e1, Y):
                    return Add(e2, Mul(Int(2),Y()).simplify()).simplify()
                if isinstance(e2, Y):
                    return Add(e1, Mul(Int(2),Y()).simplify()).simplify()
            if isinstance(simp2,Add):
                o1=simp2.p1
                o2=simp2.p2
                if isinstance(e1, Int):
                ...
                if isinstance(e1, Y):
                    if isinstance(o1, Y):
                        return Add(Mul(Int(2),Y()).simplify(), Add(o2,e2).simplify()).simplify()
                    if isinstance(o2, Y):
                        return Add(Mul(Int(2),Y()).simplify(), Add(o1,e2).simplify()).simplify()
                if isinstance(e2, Y):
                    if isinstance(o1, Y):
                        return Add(Mul(Int(2),Y()).simplify(), Add(o2,e1).simplify()).simplify()
                    if isinstance(o2, Y):
                        return Add(Mul(Int(2),Y()).simplify(), Add(o1,e1).simplify()).simplify()
        if isinstance(simp2, Add):
            e1=simp2.p1
            e2=simp2.p2
            if isinstance(simp1, Mul):
...
poly5 = Sub(Add(Mul( X(), Mul( Y(), Int(9))),Int(4)),Int(-1))
